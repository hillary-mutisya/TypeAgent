...
describe('EntityGraphManager', () => {
    ...
    test('should add relationships between entities', () => {
        ...
        if (relationships && relationships.length > 0 && relationships[0]) {
            ...
            console.log('Debug: graph:', JSON.stringify(graphManager.getGraph(), null, 2));
        }
    });
    ...
    test('should get entity relationships', () => {
        ...
        graphManager.addRelationship('Test Entity', 'Related Entity', relationship);

        const relationships = graphManager.getEntityRelationships('Test Entity');
