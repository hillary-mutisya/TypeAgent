        ...
        const commands = [
          {
            ...
            handler: () => {
              console.log('🧪 Test continue command triggered on Enter')
              executeSlashCommand('continue', {}, true, view, lineStart, lineEnd)
            }
          },
          {
            pattern: /^\/continue\s*$/,
            handler: () => {
              console.log('🤖 Continue command triggered on Enter')
              executeSlashCommand('continue', {}, false, view, lineStart, lineEnd)
            }
          },
          {
            pattern: /^\/test:diagram(?:\s+(.+))?\s*$/,
            handler: (match: RegExpMatchArray) => {
              const description = match[1]?.trim() || 'test diagram'
              console.log('🧪 Test diagram command triggered on Enter with description:', description)
              executeSlashCommand('diagram', { description }, true, view, lineStart, lineEnd)
            }
          },
          {
            pattern: /^\/diagram\s+(.+)\s*$/,
            handler: (match: RegExpMatchArray) => {
              ...
              console.log('📊 Diagram command triggered on Enter with description:', description)
              executeSlashCommand('diagram', { description }, false, view, lineStart, lineEnd)
            }
          },
          {
            pattern: /^\/test:augment(?:\s+(.+))?\s*$/,
            handler: (match: RegExpMatchArray) => {
              const instruction = match[1]?.trim() || 'improve formatting'
              console.log('🧪 Test augment command triggered on Enter with instruction:', instruction)
              executeSlashCommand('augment', { instruction }, true, view, lineStart, lineEnd)
            }
          },
          {
            pattern: /^\/augment\s+(.+)\s*$/,
            handler: (match: RegExpMatchArray) => {
              ...
              executeSlashCommand('augment', { instruction }, false, view, lineStart, lineEnd)
            }
          }
        ]
...
async function executeSlashCommand(
  ...
  setTimeout(() => {
    if (typeof window !== 'undefined' && (window as any).executeAgentCommand) {
  ...
  return new Plugin({
    ...
    props: {
      decorations(state: any) {
        ...
        const slashCommands = [
          { pattern: /^\/test:continue$/, preview: ' (press Enter to continue with AI test mode)' },
          { pattern: /^\/continue$/, preview: ' (press Enter to continue with AI)' },
          { pattern: /^\/test:diagram$/, preview: ' [description] (press Enter for test diagram)' },
          { pattern: /^\/diagram$/, preview: ' <description> (press Enter to generate diagram)' },
          { pattern: /^\/test:augment$/, preview: ' [instruction] (press Enter for test augmentation)' },
          { pattern: /^\/augment$/, preview: ' <instruction> (press Enter to augment document)' }
        ]
