...
export type EntityType = 'person' | 'organization' | 'product' | 'concept' | 'location' | 'technology' | 'event' | 'document';
...
export class RealTimeEntityExtractor {
     ...
     */
    async extractEntitiesFromWebsites(websites: Website[]): Promise<EnhancedEntity[]> {
        ...
        for (const website of websites) {
            try {
                ...
                console.warn(`Failed to extract entities from ${website.url}:`, error);
            }
        }
     ...
     */
    async buildEntityRelationships(entities: EnhancedEntity[], websites: Website[]): Promise<EntityRelationship[]> {
        const relationships: EntityRelationship[] = [];
        const entityNames = new Set(entities.map(e => e.name.toLowerCase()));

        for (const website of websites) {
            try {
                const siteRelationships = await this.extractRelationshipsFromWebsite(website, entityNames);
                relationships.push(...siteRelationships);
            } catch (error) {
                console.warn(`Failed to extract relationships from ${website.url}:`, error);
            }
        }
     ...
     */
    async updateEntityGraph(newWebsites: Website[]): Promise<EntityKnowledgeGraph> {
        ...
        const relationships = await this.buildEntityRelationships(entities, newWebsites);
        ...
        console.log(`Entity graph updated in ${Date.now() - startTime}ms: ${entities.length} entities, ${relationships.length} relationships`);
        return graph;
    }
     ...
     */
    async migrateFromExistingKnowledge(websiteCollection: WebsiteCollection): Promise<EntityKnowledgeGraph> {
        try {
            // Get existing entities from the website collection
            const existingEntities = await this.getExistingEntities(websiteCollection);
            const websites = await this.getWebsitesFromCollection(websiteCollection);

            // Enhance existing entities with additional data
            const enhancedEntities: EnhancedEntity[] = [];
            for (const entity of existingEntities) {
                const enhanced = await this.enhanceEntityWithWebsiteData(entity, websites);
                enhancedEntities.push(enhanced);
            }

            // Extract additional entities that might have been missed
            const newEntities = await this.extractEntitiesFromWebsites(websites);
            
            // Merge existing and new entities
            const allEntities = this.mergeEntityLists(enhancedEntities, newEntities);
            
            // Build relationships
            const relationships = await this.buildEntityRelationships(allEntities, websites);
     ...
     */
    async enhanceEntityWithWebsiteData(entity: Entity, websites: Website[]): Promise<EnhancedEntity> {
    ...
    private async extractFromWebsite(website: Website): Promise<ExtractionResult> {
        ...
        const patternEntities = this.extractEntitiesWithPatterns(content, website);
        ...
        const contextEntities = this.extractEntitiesFromContext(content, website);

        // Combine and deduplicate
        const allEntities = [...patternEntities, ...nerEntities, ...contextEntities];
        ...
        const result: ExtractionResult = {
            ...
            extractionQuality: this.calculateExtractionQuality(uniqueEntities, content)
        };
    ...
    private initializeStopWords(): void {
        const words = ['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should'];
        this.stopWords = new Set(words);
    }

    private extractEntitiesWithPatterns(content: string, website: Website): EnhancedEntity[] {
        ...
        for (const [type, patterns] of this.entityPatterns) {
            for (const pattern of patterns) {
                ...
                for (const match of matches) {
                    ...
                    if (entityName && !this.stopWords.has(entityName.toLowerCase())) {
                        entities.push(this.createEnhancedEntity(entityName, type, website, 'pattern'));
                    }
                }
            }
        }
    ...
    private extractEntitiesWithNER(content: string, website: Website): EnhancedEntity[] {
        ...
        for (const word of words) {
            ...
            if (cleanWord && cleanWord[0] === cleanWord[0].toUpperCase() && cleanWord.length > 2) {
                currentSequence = currentSequence ? `${currentSequence} ${cleanWord}` : cleanWord;
            } else {
        ...
        for (const sequence of capitalizedSequences) {
            ...
            entities.push(this.createEnhancedEntity(sequence, inferredType, website, 'nlp'));
        }

        return entities;
    }

    private extractEntitiesFromContext(content: string, website: Website): EnhancedEntity[] {
        // Extract entities based on context clues
        const entities: EnhancedEntity[] = [];
        
        // Technology context
        const techRegex = /using\s+(\w+(?:\s+\w+)*)|built\s+with\s+(\w+(?:\s+\w+)*)|powered\s+by\s+(\w+(?:\s+\w+)*)/gi;
        const techMatches = content.matchAll(techRegex);
        for (const match of techMatches) {
            const tech = match[1] || match[2] || match[3];
            if (tech) {
                entities.push(this.createEnhancedEntity(tech, 'technology', website, 'pattern'));
            }
        }
    ...
    private createEnhancedEntity(name: string, type: EntityType, website: Website, method: 'nlp' | 'pattern' | 'manual' | 'hybrid'): EnhancedEntity {
        return {
            name: name.trim(),
            type,
            ...
            strongRelationships: [],
            coOccurringEntities: [],
            contextSnippets: [this.extractContextSnippet(name, website.content || website.title || '')],
            topicAffinity: [],
    ...
    private inferEntityType(text: string): EntityType {
        ...
        // Technology keywords
        if (/\b(js|react|vue|angular|node|python|java|docker|api|sdk|framework|library)\b/.test(lowerText)) {
            return 'technology';
        }
        
        // Organization keywords
        if (/\b(inc|corp|llc|company|ltd|organization|foundation|institute)\b/.test(lowerText)) {
            return 'organization';
        }
    ...
    private calculateEntityConfidence(name: string, type: EntityType, method: 'nlp' | 'pattern' | 'manual' | 'hybrid'): number {
        let confidence = 0.5; // Base confidence
        
        // Boost based on extraction method
        switch (method) {
            case 'manual': confidence = 0.95; break;
            case 'pattern': confidence = 0.8; break;
            case 'nlp': confidence = 0.7; break;
            case 'hybrid': confidence = 0.85; break;
        }
        ...
        if (type === 'technology' && /^(React|Vue|Angular|TypeScript|JavaScript|Python|Java)$/i.test(name)) {
            confidence += 0.15;
        }
    ...
    private async extractRelationshipsFromWebsite(website: Website, entityNames: Set<string>): Promise<EntityRelationship[]> {
        const relationships: EntityRelationship[] = [];
        const content = website.content || website.title || '';
                ...
                for (const match of matches) {
                    ...
                    if (entity1 && entity2 && 
                        ...
                        relationships.push({
                            ...
                            firstObserved: website.timestamp || new Date().toISOString(),
                            lastObserved: website.timestamp || new Date().toISOString(),
                            strength: 0.7,
    ...
    private mergeEntities(existing: EnhancedEntity, newEntity: EnhancedEntity): void {
    ...
    private consolidateRelationships(relationships: EntityRelationship[]): EntityRelationship[] {
        ...
        for (const rel of relationships) {
            ...
            if (existing) {
                ...
                existing.evidenceSources = [...new Set(existing.evidenceSources)];
    ...
    private buildEntityIndex(entities: EnhancedEntity[]): Map<EntityType, string[]> {
    ...
    private groupRelationshipsByEntity(relationships: EntityRelationship[]): Map<string, EntityRelationship[]> {
    ...
    private findEntityMentions(entityName: string, website: Website): string[] {
        ...
        while ((match = regex.exec(content)) !== null) {
            ...
            const end = Math.min(content.length, match.index + entityName.length + 30);
    ...
    private extractAliases(entityName: string, contextSnippets: string[]): string[] {
        const aliases: string[] = [];
        
        // Look for common alias patterns in context
        for (const snippet of contextSnippets) {
            // Pattern: "Entity (Alias)" or "Alias (Entity)"
            const aliasPattern = new RegExp(`${entityName}\\s*\\(([^)]+)\\)|\\(([^)]+)\\)\\s*${entityName}`, 'gi');
    ...
    private extractTopics(contextSnippets: string[]): string[] {
        const topics: string[] = [];
        const topicKeywords = [
            'technology', 'business', 'science', 'research', 'development',
            'innovation', 'software', 'hardware', 'artificial intelligence',
            'machine learning', 'data science', 'web development', 'mobile',
            'cloud computing', 'cybersecurity', 'blockchain', 'fintech'
        ];
    ...
    private calculateExtractionQuality(entities: EnhancedEntity[], content: string): number {
        ...
        const entityDensity = Math.min(1, entities.length / (content.length / 100)); // Entities per 100 chars
        
        return (avgConfidence * 0.7) + (entityDensity * 0.3);
    }

    private mergeEntityLists(list1: EnhancedEntity[], list2: EnhancedEntity[]): EnhancedEntity[] {
        const merged = [...list1];
        
        for (const entity of list2) {
            const existing = merged.find(e => e.name.toLowerCase() === entity.name.toLowerCase());
            if (existing) {
                this.mergeEntities(existing, entity);
            } else {
                merged.push(entity);
    ...
    private async getExistingEntities(websiteCollection: WebsiteCollection): Promise<Entity[]> {
    ...
    private async getWebsitesFromCollection(websiteCollection: WebsiteCollection): Promise<Website[]> {
