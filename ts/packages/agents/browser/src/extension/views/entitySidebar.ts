...
export class EntitySidebar {
     ...
     */
    async loadEntity(entityData: EntityData | string, fullEntityData?: any): Promise<void> {
        this.currentEntity = fullEntityData || this.createMockEntityData(entityData);
    ...
    private renderEntityMetrics(): void {
        ...
        if (relationshipsEl) {
            const relationshipCount = this.currentEntity.strongRelationships?.length || 0;
    ...
    private renderAliases(): void {
        ...
        if (this.currentEntity.aliases.length === 0) {
            aliasesList.innerHTML = '<span class="empty-message">No aliases</span>';
            return;
        }

        const aliasesHtml = this.currentEntity.aliases
            .map((alias: string) => `<span class="alias-tag">${this.escapeHtml(alias)}</span>`)
            .join('');
    ...
    private renderDomains(): void {
        ...
        if (this.currentEntity.dominantDomains.length === 0) {
            domainsList.innerHTML = '<span class="empty-message">No domains</span>';
            return;
        }

        const domainsHtml = this.currentEntity.dominantDomains
            .map((domain: string) => `<span class="domain-tag">${this.escapeHtml(domain)}</span>`)
            .join('');
    ...
    private renderTopics(): void {
        ...
        if (this.currentEntity.topicAffinity.length === 0) {
            topicsList.innerHTML = '<span class="empty-message">No topics</span>';
            return;
        }

        const topicsHtml = this.currentEntity.topicAffinity
            .map((topic: string) => `<span class="topic-tag">${this.escapeHtml(topic)}</span>`)
            .join('');
    ...
    private renderEntityTimeline(): void {
        ...
        if (firstSeenEl) {
            firstSeenEl.textContent = this.formatDate(this.currentEntity.firstSeen);
        }

        if (lastSeenEl) {
            lastSeenEl.textContent = this.formatDate(this.currentEntity.lastSeen);
        }
    }

    private renderRelatedEntities(): void {
        const relatedEntitiesList = document.getElementById('relatedEntitiesList');
        if (!relatedEntitiesList || !this.currentEntity.strongRelationships) return;

        if (this.currentEntity.strongRelationships.length === 0) {
            relatedEntitiesList.innerHTML = '<div class="empty-message">No related entities</div>';
            return;
        }
        ...
        // Add click handlers
        relatedEntitiesList.querySelectorAll('.related-entity-item').forEach(item => {
    ...
    private renderRelatedEntityItem(relationship: any): string {
        const strengthClass = this.getRelationshipStrengthClass(relationship.strength);
    ...
    private updateRelationshipTypeFilter(): void {
        const filterSelect = document.getElementById('relationshipTypeFilter') as HTMLSelectElement;
        if (!filterSelect || !this.currentEntity.strongRelationships) return;

        // Get unique relationship types
        const relationshipTypes = new Set<string>(
            this.currentEntity.strongRelationships.map((rel: any) => rel.relationshipType)
        );
    ...
    private renderEmptyState(): void {
        ...
        if (relatedEntitiesList) {
            relatedEntitiesList.innerHTML = '<div class="empty-message">Select an entity to see related items</div>';
        }
        ...
        if (aliasesSection) {
            const aliasesList = aliasesSection.querySelector('.aliases-list');
            if (aliasesList) aliasesList.innerHTML = '<span class="empty-message">No aliases</span>';
        }

        if (domainsSection) {
            const domainsList = domainsSection.querySelector('.domains-list');
            if (domainsList) domainsList.innerHTML = '<span class="empty-message">No domains</span>';
        }

        if (topicsSection) {
            const topicsList = topicsSection.querySelector('.topics-list');
            if (topicsList) topicsList.innerHTML = '<span class="empty-message">No topics</span>';
        }
    ...
    private createMockEntityData(nodeData: EntityData | string): any {
        ...
        const type = typeof nodeData === 'string' ? 'organization' : nodeData.type;
        const confidence = typeof nodeData === 'string' ? 0.8 : nodeData.confidence;
        ...
        const mockData = {
            ...
            importance: typeof nodeData === 'object' && nodeData.importance ? nodeData.importance : 0.5,
            clusterGroup: typeof nodeData === 'object' && nodeData.clusterGroup ? nodeData.clusterGroup : 'general',
    ...
    private generateMockAliases(name: string, type: string): string[] {
        ...
        if (type === 'technology') {
            ...
            if (name === 'Elon Musk') {
                aliases.push('@elonmusk', 'Elon Reeve Musk');
            }
        }
    ...
    private generateMockDomains(type: string): string[] {
        const domainMap: { [key: string]: string[] } = {
            technology: ['github.com', 'stackoverflow.com', 'docs.microsoft.com'],
            organization: ['techcrunch.com', 'bloomberg.com', 'cnbc.com'],
            person: ['twitter.com', 'linkedin.com', 'wikipedia.org'],
    ...
    private generateMockTopics(type: string): string[] {
        const topicMap: { [key: string]: string[] } = {
            technology: ['software development', 'web development', 'programming'],
    ...
    private generateMockRelationships(entityName: string, entityType: string): any[] {
        // Generate contextual mock relationships based on entity
        const relationships: any[] = [];

        if (entityName === 'React') {
            relationships.push(
                { relatedEntity: 'Facebook', relationshipType: 'created_by', strength: 0.9, confidence: 0.95 },
                { relatedEntity: 'Next.js', relationshipType: 'framework_for', strength: 0.8, confidence: 0.9 },
                { relatedEntity: 'TypeScript', relationshipType: 'commonly_used_with', strength: 0.7, confidence: 0.85 }
            );
        } else if (entityName === 'Tesla') {
            relationships.push(
                { relatedEntity: 'Elon Musk', relationshipType: 'led_by', strength: 0.9, confidence: 0.95 },
                { relatedEntity: 'Model 3', relationshipType: 'manufactures', strength: 0.95, confidence: 0.98 },
                { relatedEntity: 'SpaceX', relationshipType: 'sister_company', strength: 0.6, confidence: 0.8 }
            );
        } else {
            // Generate generic relationships
            relationships.push(
                { relatedEntity: 'Related Entity 1', relationshipType: 'related_to', strength: 0.6, confidence: 0.7 },
                { relatedEntity: 'Related Entity 2', relationshipType: 'connected_to', strength: 0.4, confidence: 0.6 }
            );
        }
    ...
    setupFilters(onFilterChange?: (filters: any) => void): void {
        const relationshipTypeFilter = document.getElementById('relationshipTypeFilter') as HTMLSelectElement;
        const entityTypeFilter = document.getElementById('entityTypeFilter') as HTMLSelectElement;
    ...
    private applyRelatedEntitiesFilter(): void {
        ...
        const relationshipTypeFilter_value = relationshipTypeFilter?.value || '';
        ...
        if (relationshipTypeFilter_value) {
            filteredRelationships = filteredRelationships.filter(
                (rel: any) => rel.relationshipType === relationshipTypeFilter_value
            );
        }
        ...
        if (filteredRelationships.length === 0) {
            relatedEntitiesList.innerHTML = '<div class="empty-message">No matching relationships</div>';
        } else {
