...
export class EnhancedSearchWebMemories {
     ...
     */
    async searchByEntity(entityName: string, options: EntitySearchOptions = {}): Promise<EntitySearchResult> {
        ...
        try {
            ...
            if (websiteSearchResults.relatedEntities) {
                ...
                for (const entity of websiteSearchResults.relatedEntities) {
                    if (entity.name.toLowerCase().includes(entityName.toLowerCase()) || 
                        ...
                        const enhancedEntity = await this.convertToEnhancedEntity(entity, websiteSearchResults.websites);
            ...
            if (foundEntities.length === 0) {
                const extractedEntities = await this.entityExtractor.extractEntitiesFromWebsites(
                    websiteSearchResults.websites.map(w => ({
                        url: w.url,
                        title: w.title,
                        content: w.snippet || '',
                        timestamp: w.lastVisited || new Date().toISOString()
                    }))
                );

                foundEntities.push(...extractedEntities.filter(e => 
     ...
     */
    async getEntityGraph(centerEntity: string, depth: number = 2): Promise<EntityGraphData> {
        const startTime = Date.now();
        ...
        try {
            ...
            const centerEntityResult = await this.searchByEntity(centerEntity, { maxResults: 1 });
                ...
                for (const entity of currentEntities) {
                    ...
                    if (relatedSearch.relatedEntities) {
                        for (const relatedEntity of relatedSearch.relatedEntities) {
                            if (!allEntities.some(e => e.name === relatedEntity.name)) {
                                const enhancedRelated = await this.convertToEnhancedEntity(relatedEntity, relatedSearch.websites);
                                ...
                                allRelationships.push({
                                    ...
                                    evidenceSources: relatedSearch.websites.slice(0, 3).map(w => w.url),
     ...
     */
    async getEntityData(entityName: string, useMockData: boolean = this.mockMode): Promise<EnhancedEntity | null> {
        if (useMockData) {
        ...
        const searchResult = await this.searchByEntity(entityName, { maxResults: 1 });
        return searchResult.entities.length > 0 ? searchResult.entities[0] : null;
    }

    /**
     * Refresh entity data from source
     */
    async refreshEntityData(entityName: string): Promise<EnhancedEntity | null> {
     ...
     */
    private async convertToEnhancedEntity(entity: Entity, websites: any[]): Promise<EnhancedEntity> {
        return {
            name: entity.name,
            ...
            dominantDomains: websites.slice(0, 3).map(w => new URL(w.url).hostname),
            strongRelationships: [],
            coOccurringEntities: [],
            contextSnippets: websites.slice(0, 3).map(w => w.snippet || w.title).filter(s => s),
            topicAffinity: [],
    ...
    private async getEntitySuggestions(entityName: string): Promise<string[]> {
        ...
        return ['Related entities', 'Similar organizations', 'Connected people'];
    }
     ...
     */
    private searchMockEntities(entityName: string, options: EntitySearchOptions, startTime: number): EntitySearchResult {
        ...
        return {
            ...
            searchTime: Date.now() - startTime,
            cacheHit: false,
            suggestions: ['Tesla', 'SpaceX', 'OpenAI'],
            filters: {
                availableTypes: ['organization', 'person', 'product'],
                availableDomains: ['tesla.com', 'spacex.com', 'openai.com'],
                dateRange: {
                    earliest: '2020-01-01T00:00:00.000Z',
                    latest: new Date().toISOString()
                }
     ...
     */
    private getMockEntityGraph(centerEntity: string, depth: number, startTime: number): EntityGraphData {
        const entities = this.getMockEntities();
        ...
        return [
            {
                ...
                strongRelationships: [],
                coOccurringEntities: [],
                contextSnippets: ['Tesla is an electric vehicle manufacturer'],
                topicAffinity: ['electric vehicles'],
                sourceWebsites: ['https://tesla.com'],
                extractionMethod: 'hybrid',
                lastUpdated: new Date().toISOString()
            },
            {
                ...
                firstSeen: '2020-01-01T00:00:00.000Z',
                lastSeen: new Date().toISOString(),
                dominantDomains: ['twitter.com'],
                strongRelationships: [],
                coOccurringEntities: [],
                contextSnippets: ['Elon Musk is the CEO of Tesla'],
                topicAffinity: ['entrepreneurship'],
                sourceWebsites: ['https://twitter.com/elonmusk'],
                extractionMethod: 'hybrid',
                lastUpdated: new Date().toISOString()
            }
        ];
