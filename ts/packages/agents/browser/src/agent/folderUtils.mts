...
export async function enumerateHtmlFiles(
    ...
    try {
        ...
        throw new Error(`Failed to enumerate files in ${folderPath}: ${(error as Error).message}`);
    }
}
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (options.recursive) {
                await enumerateFilesRecursive(fullPath, files, options, depth + 1);
            }
        } else if (entry.isFile()) {
            // Check if file matches our criteria
            if (isHtmlFile(fullPath, options) && await isValidFileSize(fullPath, options)) {
                files.push(fullPath);
 ...
 */
async function isValidFileSize(filePath: string, options: FolderOptions): Promise<boolean> {
    try {
        const stats = await fs.promises.stat(filePath);
        const maxSize = options.maxFileSize || DEFAULT_FOLDER_OPTIONS.maxFileSize!;
 ...
 */
export function applyFileFilters(files: string[], options: FolderOptions): string[] {
    let filtered = [...files];
 ...
 */
export async function getFolderStats(folderPath: string, options: FolderOptions = DEFAULT_FOLDER_OPTIONS): Promise<{
    totalFiles: number;
    htmlFiles: number;
    totalSize: number;
    ...
    return {
        ...
        totalSize,
        averageFileSize: htmlFiles.length > 0 ? totalSize / htmlFiles.length : 0,
        largestFile
    };
}
 ...
 */
export async function validateHtmlFolder(folderPath: string, options: FolderOptions = DEFAULT_FOLDER_OPTIONS): Promise<ValidationResult> {
    ...
    try {
        ...
        const htmlFiles = await enumerateHtmlFiles(folderPath, { ...options, limit: 1 });
 ...
 */
export function createFileBatches(files: string[], batchSize: number = 10): string[][] {
